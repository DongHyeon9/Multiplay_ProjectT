...
private:
	...
	TSet<const AIG_PlayerController*> compeletedPlayers{};
...
public:
	...
	void OnEnterPlayer(const AIG_PlayerController* _PC);
	// 모든 플레이어가 시작 이벤트가 끝났으면 게임을 시작한다
	void RequestStartGame(const AIG_PlayerController* _PC);
	// 모든 플레이어가 시작 이벤트가 끝났으면 게임을 시작한다
	void OnGameOverPlayer(const AIG_PlayerController* _PC);
...
private:
	...
	bool Ticker_GameTimer(float _DeltaTime);
	// 게임을 시작한다
	void StartGame();
	// 게임을 종료한다
	void EndGame();
	...
	// @param _DeltaTime 
	// @return true면 다음에도 실행된다
	bool Ticker_EnemySpawn(float _DeltaTime);
	// 풀의 크기를 늘린다
	void ExpandPool();
