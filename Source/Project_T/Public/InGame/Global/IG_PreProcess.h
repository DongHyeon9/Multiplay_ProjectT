...
#define PTT_ERRER(Expr,...) {if(!(Expr)){PTT_LOG(Error,TEXT("ASSERTION : %s"),TEXT("'"#Expr"'"));return __VA_ARGS__;}}
...
#define SERVER_ONLY_COMP(ret) if (!GetOwner()->HasAuthority()) return ret
#define SERVER_ONLY(ret) if (!HasAuthority()) return ret

#define BROAD_CAST_CHANGE(delegate,currentData,newData)		\
{\
auto prev{currentData};					\
currentData=newData;					\
delegate.Broadcast(prev,currentData);	\
}
