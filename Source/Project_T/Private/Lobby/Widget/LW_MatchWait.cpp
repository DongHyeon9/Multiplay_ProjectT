...
namespace LOBBY_MATCH_WAIT
{
	static constexpr const TCHAR* CONNECT_STATE_PATH{ TEXT("/Game/01_Blueprint/Lobby/Widget/WG_L_ConnectState") };
}

ULW_MatchWait::ULW_MatchWait(const FObjectInitializer& _Initializer)
	:Super(_Initializer)
{
	static ConstructorHelpers::FClassFinder<UUserWidget> CONNECT_STATE_WIDGET(LOBBY_MATCH_WAIT::CONNECT_STATE_PATH);
...
void ULW_MatchWait::NativeConstruct()
{
	...
	refreshHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateUObject(this, &ULW_MatchWait::Ticker_RefreshMatchState), refreshInterval);
...
bool ULW_MatchWait::Ticker_RefreshMatchState(float _DeltaTime)
{
